...
// Derive the parent comic slug from a chapter slug, e.g.
// "one-piece-chapter-1105" -> "one-piece"
const deriveComicSlug = (chapterSlug) =>
  (chapterSlug || '').replace(/-chapter-\d+(?:[.-]\d+)?$/i, '');
...
const KomikReader = () => {
  ...
  useEffect(() => {
    ...
    const fetchChapter = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setChapter(res);
          const comicSlug = deriveComicSlug(chapterSlug);
          const rawComic = res?.raw?.comic || res?.raw?.manga || res?.raw || {};
          addToKomikHistory(user?.uid, {
            komikId: comicSlug,
            komikTitle: rawComic.title || rawComic.name || humanizeSlug(comicSlug),
            chapterSlug,
            chapterTitle: res?.title || chapterSlug,
            cover: rawComic.cover || rawComic.poster || rawComic.thumbnail || '',
          });
        }
      } catch (err) {
  ...
  // Beri EXP begitu chapter selesai dibaca: halaman terakhir sudah termuat
  // (mode Gulir) atau pembaca sudah sampai di halaman terakhir (mode
  // Halaman). Idempoten lewat marker `expLog/komik_*`, jadi aman dipanggil
  // berkali-kali (mis. saat scroll naik-turun di halaman terakhir).
  useEffect(() => {
    if (!chapterSlug || !user?.uid || images.length === 0) return;
        ...
        {mode === 'vertical' ? (
          ...
          <div className="kr-horizontal">
            ...
            <div className="kr-horizontal__info">
              <span className="kr-horizontal__hint">← →</span>
            </div>
          </div>
        )}
      </main>
