...
const DonghuaDetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await animeAPI.getDonghuaOngoing(1);
        const list = res?.ongoing_donghua || res?.data || [];
        if (cancelled) return;
        const norm = (s) => String(s || '').replace(/\/$/, '');
        setRecommendations(
          list.filter((it) => norm(it.slug) !== norm(slug)).slice(0, 12),
        );
      } catch { /* ignore */ }
    };
    load();
  ...
  if (error ||!donghua) {
    return (
      <div className="error-container main-container">
        ...
        <p className="error-hint">Mungkin konten belum tersedia.</p>
          ...
          <section className="section section-neo" aria-labelledby="dd-rec-heading">
            ...
            <div className="anime-grid">
              {recommendations.map((item, idx) => (
                <AnimeCard
                  key={item.slug ?? idx}
                  anime={{ ...item, animeId: item.slug, provider: 'donghua' }}
                  index={idx}
