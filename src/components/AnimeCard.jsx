...
export const normalizeAnime = (anime, index = 0) => {
  ...
  if (typeof rawProvider === 'string' && rawProvider.trim() !== '') {
    ...
    if (p.includes('donghua')) providerLabel = 'Donghua';
    else providerLabel = 'FiveNime'; // semua provider (otakudesu, samehadaku, custom, stream, default) → FiveNime
  }
...
const AnimeCard = ({ anime, index = 0, innerRef, statusOverride, providerHint }) => {
  ...
  // Normalisasi providerHint: ganti Otakudesu/Samehadaku → FiveNime
  const normalizeHint = (hint) => {
    if (!hint) return hint;
    const h = hint.toLowerCase();
    if (h.includes('donghua')) return 'Donghua';
    return 'FiveNime';
  };

  const finalProvider = normalizeHint(providerHint) || providerLabel || null;
