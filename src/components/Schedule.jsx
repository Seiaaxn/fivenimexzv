...
const Schedule = () => {
  ...
  const scrollToDay = (dayName) => {
    ...
    // langsung tampilkan hari tersebut & posisikan ke daftar animenya
    requestAnimationFrame(() => {
      const el = dayRefs.current[dayName];
      if (el) {
        const offset = 90; // account for sticky tabs height
  ...
  return (
    <div className="schedule-page main-container">
      ...
      {days.length > 0 ? (
        days
          .filter((d) => !activeDay || d.day === activeDay)
          .map((dayItem, idx) => {
